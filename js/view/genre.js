...
import artistTemplate from "./level-view";
...
const genreTemplate = getElementfromTemplate(template, `game game--genre template__content`);

const btnSubmit = genreTemplate.querySelector(`.game__submit`);
btnSubmit.setAttribute(`disabled`, ``);

const gameInput = genreTemplate.querySelectorAll(`.game__input`);
for (let i = 0; i < gameInput.length; i++) {
  gameInput[i].addEventListener(`change`, () => {
    for (let j = 0; j < gameInput.length; j++) {
      if (gameInput[j].checked) {
...
btnSubmit.addEventListener(`click`, () => {
  renderScreen(artistTemplate);
});

const gameBack = genreTemplate.querySelector(`.game__back`);
